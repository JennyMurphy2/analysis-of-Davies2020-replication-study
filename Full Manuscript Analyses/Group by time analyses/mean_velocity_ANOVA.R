...
mean_data_45 <- rep_data %>%
  ...
  select(id, group, pre_mv_rep_1:post_mv_rep_2) # only interested in mean velocity right now


## Use the best of the two trials for analyses as per original paper

mean_data_45 <- mean_data_45 %>% 
  rowwise() %>%
  mutate(pre = max(pre_mv_rep_1, pre_mv_rep_2, na.rm = TRUE),
         post = max(post_mv_rep_1, post_mv_rep_2, na.rm = TRUE)) %>%
  as.data.frame()

# Long format data

anova_mean_45 <- mean_data_45 %>%
  select(-c(pre_mv_rep_1:post_mv_rep_2)) %>%
  pivot_longer(cols = c(pre, post),
               names_to = "time",
               values_to = "mean_velocity") 

anova_mean_45$group <-  as.factor(anova_mean_45$group)
anova_mean_45$time <-  as.factor(anova_mean_45$time)


## Descriptives ---------------------

summary_45 <- anova_mean_45 %>%
  group_by(group, time) %>%
  summarise(count = n (),
            mean = mean(mean_velocity),
            sd = sd(mean_velocity))
summary_45
...
hist_45 <- anova_mean_45 %>%
  select(mean_velocity)
...
hist_45_plot <- ggplot(data = hist_45, aes(x = value, fill = variable)) +
  geom_histogram(color = "black", fill = "white",
                 bins = 15) +
  facet_wrap( ~ variable) +
  scale_x_continuous(name = "mean velocity")
hist_45_plot

### Q-Q plots 

ggplot(anova_mean_45, aes(sample = mean_velocity)) +
  geom_qq() +
  geom_qq_line() +
...
ggplot(anova_mean_45, aes(x = time, y = mean_velocity)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = .2)

## 45% mean velocity ANOVA -----------------------------------------

##afex::aov_4(continuous_var ~ group_var + (RM_var|id_var)

anova_45_afx <- afex::aov_4(
  mean_velocity ~ group * time + (time | id),
  data = anova_mean_45,
...
anova_mean_45 %>% 
  dplyr::group_by(time) %>% 
  rstatix::shapiro_test(mean_velocity)
...
anova_mean_45 %>%
  group_by(time) %>%
  rstatix::identify_outliers(mean_velocity)
...
anova_mean_55 <- mean_data_55 %>%
  select(-c(pre_mv_rep_1:post_mv_rep_2)) %>%
  pivot_longer(cols = c(pre, post),
               names_to = "time",
               values_to = "mean_velocity") 

anova_mean_55$group <-  as.factor(anova_mean_55$group)
anova_mean_55$time <-  as.factor(anova_mean_55$time)



## Descriptives ---------------------

summary_55 <- anova_mean_55 %>%
  group_by(group, time) %>%
  summarise(count = n (),
            mean = mean(mean_velocity),
            sd = sd(mean_velocity))
...
ggplot(anova_mean_55, aes(sample = mean_velocity)) +
  geom_qq() +
  geom_qq_line() +
  scale_x_continuous(name = "Observed Value") +
  scale_y_continuous(name = "Expected Normal")
...
ggplot(anova_mean_55, aes(x = time, y = mean_velocity)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = .2)

## 55% mean velocity ANOVA -----------------------------------------
...
anova_mean_65 <- mean_data_65 %>%
  select(-c(pre_mv_rep_1:post_mv_rep_2)) %>%
  pivot_longer(cols = c(pre, post),
               names_to = "time",
               values_to = "mean_velocity") 

anova_mean_65$group <-  as.factor(anova_mean_65$group)
anova_mean_65$time <-  as.factor(anova_mean_65$time)



## Descriptives ---------------------

summary_65 <- anova_mean_65 %>%
  group_by(group, time) %>%
  summarise(count = n (),
            mean = mean(mean_velocity),
            sd = sd(mean_velocity))
...
ggplot(anova_mean_65, aes(sample = mean_velocity)) +
  geom_qq() +
  geom_qq_line() +
  scale_x_continuous(name = "Observed Value") +
  scale_y_continuous(name = "Expected Normal")
...
ggplot(anova_mean_65, aes(x = time, y = mean_velocity)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = .2)

## 65% mean velocity ANOVA -----------------------------------------
...
anova_mean_75 <- mean_data_75 %>%
  select(-c(pre_mv_rep_1:post_mv_rep_2)) %>%
  pivot_longer(cols = c(pre, post),
               names_to = "time",
               values_to = "mean_velocity") 

anova_mean_75$group <-  as.factor(anova_mean_75$group)
anova_mean_75$time <-  as.factor(anova_mean_75$time)
...
ggplot(anova_mean_75, aes(sample = mean_velocity)) +
  geom_qq() +
  geom_qq_line() +
  scale_x_continuous(name = "Observed Value") +
  scale_y_continuous(name = "Expected Normal")
...
ggplot(anova_mean_75, aes(x = time, y = mean_velocity)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = .2)

## 75% mean velocity ANOVA -----------------------------------------
...
anova_mean_85 <- mean_data_85 %>%
  select(-c(pre_mv_rep_1:post_mv_rep_2)) %>%
  pivot_longer(cols = c(pre, post),
               names_to = "time",
               values_to = "mean_velocity") 

anova_mean_85$group <-  as.factor(anova_mean_85$group)
anova_mean_85$time <-  as.factor(anova_mean_85$time)
...
ggplot(anova_mean_85, aes(sample = mean_velocity)) +
  geom_qq() +
  geom_qq_line() +
  scale_x_continuous(name = "Observed Value") +
  scale_y_continuous(name = "Expected Normal")
...
ggplot(anova_mean_85, aes(x = time, y = mean_velocity)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = .2)

## 85% mean velocity ANOVA -----------------------------------------
...
ggplot(anova_mean_95, aes(sample = mean_velocity)) +
  geom_qq() +
  geom_qq_line() +
  scale_x_continuous(name = "Observed Value") +
  scale_y_continuous(name = "Expected Normal")
...
ggplot(anova_mean_95, aes(x = time, y = mean_velocity)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = .2)

## 95% mean velocity ANOVA -----------------------------------------
