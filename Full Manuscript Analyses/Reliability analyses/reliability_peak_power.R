...
peak_data_45 <- rep_data %>%
  ...
  select(id, pre_pp_rep_1:post_pp_rep_2) # only interested in peak power right now

# prepare data

rep_1_45 <- peak_data_45 %>%
  select(-c(pre_pp_rep_2, post_pp_rep_2)) %>%
  pivot_longer(cols = c(pre_pp_rep_1, post_pp_rep_1),
               names_to = "time",
               values_to = "rep_1")

rep_2_45 <- peak_data_45 %>%
  select(-c(pre_pp_rep_1, post_pp_rep_1)) %>%
  pivot_longer(cols = c(pre_pp_rep_2, post_pp_rep_2),
               names_to = "time",
               values_to = "rep_2")
